...
// (这是已修复了 calculateGlobalPerformance 返回键名的完整代码)
...
class CalculatorService {
 ...
 Future<Map<String, dynamic>> calculateGlobalPerformance() async {
  ...
  // --- (*** 这是关键的修复点 ***) ---
  // 你的 UI (dashboard_page) 期望的键名是: 
  // 'netInvestment', 'profitRate', 'annualizedReturn'
  // 你的函数正在返回: 
  // (missing), 'totalProfitRate', 'globalAnnualizedReturn'
  // 我们必须修复这个 return Map 来匹配 UI 的期望。
  return {
   'totalValue': totalValueCNY,
   'totalProfit': totalProfit,
   'netInvestment': totalNetInvestmentCNY,     // <-- 修复 1: 添加了 'netInvestment'
   'profitRate': totalProfitRate,          // <-- 修复 2: 重命名键
   'annualizedReturn': globalAnnualizedReturn, // <-- 修复 3: 重命名键
  };
  // --- (*** 修复结束 ***) ---
 }
 ...
 Future<List<FlSpot>> getGlobalValueHistory() async {
  ...
  if (allUpdateDays.isEmpty) return []; // (修正：如果过滤后为空，则提前返回)
  ...
  final sortedEntries = dailyTotalValues.entries.toList()..sort((a,b) => a.key.compareTo(b.key));
 	...
 	// (*** 新增逻辑：如果计算后仍然只有1个点或0个点，我们需要处理 ***)
    if (sortedEntries.length < 2) {
      // 尝试获取当前总价值作为第二个点
      final currentPerf = await calculateGlobalPerformance(); // 调用我们刚修复的函数
